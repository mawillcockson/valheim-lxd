...
The local computer has few restrictions, and can be running any operating system that can run the LXD Client, `lxc`, which includes macOS, Windows, and Linux, though Linux will probably have the easiest time getting setup. I used Debian while developing this.
...
On most Linux distributions, and macOS, these tools are either readily available, or are already installed.
...
Additionally, `lxd init` sometimes does not run correctly when it is executed outside a login environment.

When initializing `lxd`, you can select options different from the defaults if you have specific needs, but the defaults work fine, except for the question `Would you like the LXD server to be available over the network?`: answer `yes`. These scripts assume a default port of `8443` for managing the remote host. To select the defaults, hit <kbd>Enter</kbd> without typing anything else.

Below is how I installed and configured `lxd` on a remote host running Debian:

```text
user@remote:~$ sudo apt install snapd
user@remote:~$ sudo snap install core
user@remote:~$ sudo snap install lxd
user@remote:~$ sudo -i
...
Once LXD is setup on the remote host, make sure the remote host has the port `lxd` uses for remote administration (default TCP 8443) open. Additionally, you can open the range of three ports Valheim uses. Unless you decide to choose differently, the default port range for Valheim is 2456 through 2458, UDP. The ports must all be consecutive, so if the first port is changed to 60,000, then the other two would be 60,001 and 60,002.
...
To ensure that the time on the remote host is in sync with reality, most distributions come with an NTP client already configured and running, and the command `timedatectl timesync-status` will probably show this. If that doesn't work, search for something like "`<name of distribution>` sync time".
...
Lastly, [Windows Subsystem for Linux][wsl] allows running a distribution of Linux on Windows, without having to start a virtual machine manager like VirtualBox of VMWare. If going this route, I'd recommend installing [Ubuntu][ms store ubuntu 20.04] through WSL. [Microsoft maintains instructions for this][install wsl]. Once [Ubuntu][ms store ubuntu 20.04] is installed and running, refer the instructions above and continue as if on Ubuntu. If going this route, there's no need to install any extra tools like `lxc` or `jq` on Windows directly. These should instead be installed inside Ubuntu.

The following instuctions are for using [Git for Windows][].

As long as [`lxc`][win lxc], [`jq`][download jq], and a POSIX shell are available, the scripts should run fine, _though they have not been tested on Windows_.
...
Then, change directories into the directory that was just created, and make sure the tools the scripts rely on are available inside Git Bash:
...
To provide these, create a file called `environment.yaml` in the same directory as the `install.sh` script is in. It should be filled out like so:
...
The lines starting with `#` are comments, and don't have to be copied. Feel free to add your own comments and notes.
...
If you want to use a particular seed, you need an existing copy of the game. Start the game, and create a new world with the same parameters as in `environment.yaml` (the passwords don't need to match). Stop the game, and then copy the `.fwl` and `.db` files into the same directory as the `environment.yaml` file is in.
...
1. Sets up a connection to the remote computer (use the password entered from when `lxd init` was run)
1. Downloads an official Ubuntu container image
1. Creates a new container based on that image
1. Updates the packages, installs minimal dependencies
...
1. Sets the timezone
1. Installs Valheim (the script may appear to hang during this step, but it is downloading a 1 GB file)
1. Starts the Valheim server, and automatic updates, if those were not disabled
1. Takes a snapshot (default name `just-installed`)
...
For the snapshot, [the format that LXD uses is similar to the format of `cron`][lxd snapshots], though without shortcuts like `@daily`. Websites like [cron.help](https://cron.help/#1_5_*_*_1) can help find the format for expressing a particular time.

The update check schedule is a [`systemd` `OnCalendar` directive][]. [This document describes the format][oncalendar format]. Briefly, the 4-digit year, 2-digit month, and 2-digit day of the event are given, separated by hyphens, in that order. For our purposes, `*` is useful, as it means "any", so `*-*-*` means "any year, any month, any day". Then, after a leading space, the time at which this will happen, in a 24-hour format. The time includes a 2-digit hour, 2-digit minute, and 2-digit second, all separated by colons.

As an example, the default schedule is `*-*-* 05:10:00`, which is equivalent to "every day at 5:10 AM".

It's important to ensure that the schedule for each event causes them to occur with the same frequency. For example, it probably isn't very useful to have the snapshots happen daily, while the check for updates happens every hour (besides, checking for updates causes the Valheim to stop, and it takes a while to restart, so the game wouldn't be very playable).
...
_Note: either can be selectively disabled. The [Disable Auto-Update section](#disable-auto-update) info on how to disable each one._

As an example, let's say I wanted to have the server update and the snapshot to happen at 4:30 PM every day.
...
For the Valheim update check, I would use `*-*-* 04:30:00`, and for the snapshots I would use `25 4 * * *`.

To change the schedule for updating Valheim, create a file called `extra_vars.sh`, and add a line like the following:

```sh
export VALHEIM_AUTO_UPDATE_SCHEDULE="*-*-* 04:30:00"
```

For the snapshot schedule, add a line in `environment.yaml`:
...
The automatic updating of Valheim can be disabled, as can the snapshotting, but I recommend keeping both, unless you would like to implement a backup solution for the Valheim world files.
...
This will only take effect when done before running `install.sh`.

To disable the automatic updates on a remote host that's already been configured, run the following command:
...
To disable the automatic snapshotting, add a line to the `environment.yaml` file:
...
This will also only take effect when run before running `install.sh`.
...
_Note: The `install.sh` script will still take a snapshot at the end of the installation._

#### Manual updating and snapshotting

Valheim can be updated at any time, as can a snapshot of the container be taken. Doing either of these things will not affect any of the scheduled events: the automated snapshots and updates will still happen as configured.

The following commands refer to the remote host and the container in the format `<remote host>:<container>`, without the `<` and `>`. By default, the name `valheim` is used for both. If the scripts were run directly on the remote host, following the directions in the [Running Locally section](#running-locally), then replace `<remote host>` with `local`.
...
You can enable CPU pinning for the container, though I haven't seen a noticeable difference for Valheim. In the `environment.yaml` file, add a line for `limits.cpu`, with a value that is the range of CPU cores that should be mapped one-to-one into the container. I usually leave one CPU for the host, and give all the rest to the container, assuming the host is dedicated to running just this Valheim container.

"CPU" here is the Linux kernel's concept of a CPU. To find out how many are available, on the host system, run `lscpu --extended=CPU`. On Ubuntu and Debian, this command is available in the `util-linux` package. On my host, this looks like:
...
Generally, this is the number of cores the CPU has (x2 if it has [Simultaneous Multithreading][]).
